...
void TraversableGrid::traverseAdjacentCells(int x, int y, std::function<void (int, int)> func) const
{
    for(int i = -1; i <= 1; ++i)
    {
        for(int j = -1; j <= 1; ++j)
...
void TraversableGrid::traverseCells(std::function<void (int, int)> func) const
{
